...
set :deploy_to, '/var/www/apps/museum'
...
namespace :deploy do
  ...
  task :setup do
    on roles(:all) do
      ...
      execute 'rm -rf /var/www/log/upstart'
  ...
  task :symlink do
    on roles(:all) do
      ...
      execute "rm -f #{release_path}/Procfile"
      execute "ln -s #{shared_path}/Procfile #{release_path}/Procfile"
      execute "rm -f #{release_path}/public/system"
  ...
  task :foreman_init do
    on roles(:all) do
      ...
      within current_path do
        ...
        execute "sudo mkdir /var/run/#{application}"
        # execute "sudo chown ubuntu:ubuntu /var/run/#{application}"
        # execute :bundle, "exec foreman export upstart #{foreman_temp}
        # -a #{application} -u ubuntu -l /var/www/apps/#{application}/log -d #{current_path}"
        # execute :bundle, "exec foreman export systemctl start #{foreman_temp}
        # -a #{application} -u ubuntu -l /var/www/apps/#{application}/log -d #{current_path}"
  ...
  desc 'Deploy setup rollback'
  task :unset do
    on roles(:all) do
      sudo 'rm -rf /var/run/museum'
      sudo 'rm -rf /var/www/apps/museum'
    end
  end
