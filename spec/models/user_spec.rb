    ...
    context 'when user has not authorization' do
      context 'when user already exists' do
        ...
        it 'creates authorization for user' do
          expect { described_class.find_for_oauth(auth) }.to change(user.authentications, :count).by(1)
        end

        it 'creates authorization with provider' do
          authentication = described_class.find_for_oauth(auth).authentications.first
